...
class PlanFormRequest extends FormRequest
{
    ...
    public function rules()
    {
        $rules = [
            ...
            'customer_support' =>  ['required', 'boolean',],
            'multiple_image' =>  ['required', 'boolean',],
            'badge' =>  ['required', 'boolean',],
        ];

        if ($this->method() == 'POST') {
            $rules['label'] =  ['required', 'string','alpha_spaces', 'unique:plans,label'];
        } else {
            $rules['label'] =  ['required', 'string', 'alpha_spaces', "unique:plans,label,{$this->plan->id}"];
        }
