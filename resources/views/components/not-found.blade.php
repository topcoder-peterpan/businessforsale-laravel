...
<div class="empty py-5">
    ...
    <h5 class="mt-4">{{ __('website.no_results_found') }}</h5>
    <p class="empty-subtitle text-muted">
        {{ __('website.there_is_no') }} {{ strtolower($plural) }} {{ __('website.found_in_the_page') }}
    </p>
    @if ($route)
        <div class="empty-action">
            <a href="{{ route($route) }}" class="btn btn-primary">
                ...
                {{ __('website.add_your_first') }} {{ strtolower($singuler) }}
            </a>
        </div>
    @endif
</div>
