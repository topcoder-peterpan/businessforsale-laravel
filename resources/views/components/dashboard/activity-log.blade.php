<div class="dashboard-card dashboard-card--recent">
    <h2 class="dashboard-card__title">{{ __('website.recent_activities') }}</h2>
            ...
            <li class="dashboard-card--recent__activity-item {{ $activity->type === 'App\\Notifications\\AdDeleteNotification' ? 'danger':'success' }}">
                ...
                <p class="text--body-3">
                    ...
                    @isset($activity->data['url'])
                        <a href="{{ $activity->data['url'] }}">{{ __('website.view_ad') }}</a>
                    @endisset
                </p>
            </li>
        @empty
            <li class="text-center mt-5 pt-5">
                <p class="text--body-3">{{ __('website.no_recents_activities') }}</p>
            </li>
        @endforelse
    </ul>
</div>
