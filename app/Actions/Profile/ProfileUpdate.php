...
class ProfileUpdate
{
    ...
    {
        $user = SuperAdmin::find(auth('super_admin')->id());
    ...
    {
        $password_check = Hash::check($request->current_password, auth('super_admin')->user()->password);

        if ($password_check) {
            $user = SuperAdmin::findOrFail(auth('super_admin')->id());
            $user->update([
